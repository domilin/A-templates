...
class RecommendConfig extends Component {
    ...
    componentWillMount () {
        ...
        columns = [{
            ...
            title: '商品名称',
            dataIndex: 'k_goods_name'
        }, {
            title: '推荐位图片',
            dataIndex: 'lk_recom_file',
            render: (text, record) => {
                return <div className="recom-img-box"><img src={record.lk_recom_file} /></div>
            }
        }, {
            title: '操作',
            render: (text, record) => {
                return <div className="goods-operate">
                    ...
                    <a onClick={() => dispatch(delRecommendItem({'lk_recom_id': record.lk_recom_id}, record.key))}>删除</a>
                </div>
            }
        }]
    ...
    submitForm = (e) => {
        ...
        form.validateFields((err, fieldValues) => {
            ...
            formData.append('lk_goods_id', fieldValues.lk_goods_id)
                ...
                <Form encType="multipart/form-data" id="uploadForm">
                    <FormItem label="商品ID" {...formItemLayout}>
                        {getFieldDecorator('lk_goods_id', {
                            initialValue: query.lk_goods_id ? `${query.lk_goods_id}` : ''
                        })(<Select name="goodsId" style={{'width': '100%'}}>
                            {
                                goodsList.map((item, index) => <Option value={`${item.lk_goods_id}`} key={`${item.lk_goods_id}`}>{item.lk_goods_name}</Option>)
                            }
                        </Select>)}
